...
exports.getStatsCounts = asyncWrapper(async (req, res) => {
  const [carreras, cursos, alumnos, docentes, secciones] = await Promise.all([Carrera.countDocuments(), Curso.countDocuments(), User.countDocuments({
    rol: 'ESTUDIANTE'
  }), User.countDocuments({
    rol: 'DOCENTE'
...
exports.getRecursos = asyncWrapper(async (req, res) => {
  const now = Date.now();
  ...
  const topRutas = Object.entries(frequencyMap).map(([ruta, count]) => ({
    ruta,
    count
  })).sort((a, b) => b.count - a.count).slice(0, 5);

  // Top Consultas Más Lentas (ordenadas por duración)
  const lentas = [...metrics].sort((a, b) => b.duration - a.duration).slice(0, 5).map(m => ({
  ...
  const avgResponseTime = metrics.length > 0 ? Math.round(metrics.reduce((sum, m) => sum + m.duration, 0) / metrics.length) : 0;
...
exports.getEnvironmentalImpact = (req, res) => {
  try {
    ...
    const worstEndpoint = Object.values(byRoute).sort((a, b) => b.totalCo2mg - a.totalCo2mg)[0]?.route || '-';
    const mostUsed = Object.values(byRoute).sort((a, b) => b.count - a.count)[0]?.route || '-';
    res.json({
      totalRequests,
      ...
      worstEndpoint,
      mostUsed,
      requests: withCo2.toReversed() // Más reciente primero
    });
  } catch (err) {
    res.status(500).json({
      msg: 'Error al obtener impacto ambiental'
    });
  }
};

// === CARRERAS ===
exports.createCarrera = asyncWrapper(async (req, res) => {
  const carrera = await new Carrera(req.body).save();
  res.json(carrera);
});
exports.getCarreras = asyncWrapper(async (req, res) => {
  // [OPTIMIZACIÓN 7] Las carreras cambian raramente — cachear 60s
  res.set('Cache-Control', 'private, max-age=60');
  res.json(await Carrera.find().lean());
});
exports.updateCarrera = asyncWrapper(async (req, res) => {
  const carrera = await Carrera.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  });
  res.json(carrera);
});
exports.deleteCarrera = asyncWrapper(async (req, res) => {
  const cursos = await Curso.findOne({
    carrera: req.params.id
  });
  const alumnos = await User.findOne({
    carrera: req.params.id
  });
  if (cursos || alumnos) return res.status(400).json({
    msg: 'No se puede eliminar la carrera porque tiene cursos o alumnos asignados.'
  });
...
// === ESTUDIANTES ===
exports.createEstudiante = asyncWrapper(async (req, res) => {
  const {
    email,
    password,
    nombre,
...
exports.getEstudiantes = asyncWrapper(async (req, res) => {
  const page = parseInt(req.query.page) || 1;
  const limit = parseInt(req.query.limit) || 0; // 0 = sin límite (backward compatible)
  const skip = limit > 0 ? (page - 1) * limit : 0;
  let query = User.find({
    rol: 'ESTUDIANTE'
  }).select('-password').populate('carrera', 'nombre').lean();
  if (limit > 0) {
...
exports.importarEstudiantes = asyncWrapper(async (req, res) => {
  const {
    estudiantes
  } = req.body;
  if (!estudiantes || !Array.isArray(estudiantes)) {
    return res.status(400).json({
      msg: 'Formato de importación inválido'
    });
  }
...
exports.updateEstudiante = asyncWrapper(async (req, res) => {
...
exports.deleteEstudiante = asyncWrapper(async (req, res) => {
  // Buscar si está matriculado en alguna sección
  const secciones = await Seccion.findOne({
    estudiantesMatriculados: req.params.id
  });
  if (secciones) return res.status(400).json({
    msg: 'No se puede eliminar: el alumno está matriculado en un salón.'
  });
...
// === DOCENTES ===
exports.createDocente = asyncWrapper(async (req, res) => {
  const {
    email,
    password,
    nombre,
...
exports.getDocentes = asyncWrapper(async (req, res) => {
  const page = parseInt(req.query.page) || 1;
  const limit = parseInt(req.query.limit) || 0;
  const skip = limit > 0 ? (page - 1) * limit : 0;
  let query = User.find({
    rol: 'DOCENTE'
  }).select('-password').populate('carrerasEnsenadas', 'nombre').lean();
  if (limit > 0) {
    const total = await User.countDocuments({
      rol: 'DOCENTE'
    });
...
exports.updateDocente = asyncWrapper(async (req, res) => {
  if (req.body.password) {
    req.body.password = await bcrypt.hash(req.body.password, await bcrypt.genSalt(10));
  } else {
    delete req.body.password;
...
exports.deleteDocente = asyncWrapper(async (req, res) => {
  const secciones = await Seccion.findOne({
    docente: req.params.id
  });
  if (secciones) return res.status(400).json({
    msg: 'No se puede eliminar: el docente tiene secciones asignadas.'
  });
  await User.findByIdAndDelete(req.params.id);
  res.json({
...
// === CURSOS ===
exports.createCurso = asyncWrapper(async (req, res) => {
  const existe = await Curso.findOne({
    codigo: req.body.codigo
  });
  if (existe) return res.status(400).json({
    msg: `El código "${req.body.codigo}" ya está en uso.`
  });
...
exports.getCursos = asyncWrapper(async (req, res) => {
  const {
    carreraId
  } = req.query;
  const filtro = carreraId ? {
    carrera: carreraId
  } : {};
  const cursos = await Curso.find(filtro).populate('carrera', 'nombre').sort({
    ciclo: 1,
    nombre: 1
  }).lean();
...
exports.updateCurso = asyncWrapper(async (req, res) => {
  const curso = await Curso.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true
  }).populate('carrera', 'nombre');
  if (!curso) return res.status(404).json({
    msg: 'Curso no encontrado'
  });
  res.json(curso);
});
exports.deleteCurso = asyncWrapper(async (req, res) => {
  const secciones = await Seccion.findOne({
    curso: req.params.id
  });
  if (secciones) return res.status(400).json({
    msg: 'No se puede eliminar: el curso tiene salones abiertos.'
  });
...
// === SECCIONES ===
exports.createSeccion = asyncWrapper(async (req, res) => {
  res.json(await new Seccion(req.body).save());
});

// [OPTIMIZACIÓN 1] Secciones con proyección optimizada — no enviar array completo de estudiantesMatriculados
exports.getSecciones = asyncWrapper(async (req, res) => {
  const page = parseInt(req.query.page) || 1;
  const limit = parseInt(req.query.limit) || 0;
  const skip = limit > 0 ? (page - 1) * limit : 0;
  let query = Seccion.find().populate('curso', 'nombre codigo').populate('docente', 'nombre apellidos').lean();
  if (limit > 0) {
...
exports.updateSeccion = asyncWrapper(async (req, res) => {
  res.json(await Seccion.findByIdAndUpdate(req.params.id, req.body, {
    new: true
  }));
});
exports.deleteSeccion = asyncWrapper(async (req, res) => {
  const seccion = await Seccion.findById(req.params.id);
  if (seccion && seccion.estudiantesMatriculados.length > 0) {
    return res.status(400).json({
      msg: 'No se puede eliminar: el salón ya tiene estudiantes matriculados.'
    });
  }
