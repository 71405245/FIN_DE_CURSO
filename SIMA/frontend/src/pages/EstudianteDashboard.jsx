...
function EstudianteDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [dispRes, misRes, perfilRes] = await Promise.all([
        axios.get('http://localhost:5001/api/estudiante/secciones-disponibles', config),
        axios.get('http://localhost:5001/api/estudiante/mis-secciones', config),
        axios.get('http://localhost:5001/api/estudiante/perfil', config)
      ]);
  ...
  const handleMatricular = async (seccionId) => {
    try {
      const token = localStorage.getItem('token');
      const config = { headers: { 'x-auth-token': token } };
      await axios.post('http://localhost:5001/api/estudiante/matricular', { seccionId }, config);
  ...
  const handleRectificar = async (seccionId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const config = { headers: { 'x-auth-token': token } };
      await axios.post('http://localhost:5001/api/estudiante/rectificar', { seccionId }, config);
  ...
  const handleDownloadPDF = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://localhost:5001/api/estudiante/horario-pdf', {
  ...
  const handleGenerarIA = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const res = await axios.post('http://localhost:5001/api/estudiante/generar-horario-ia', iaPrefs, {
        headers: { 'x-auth-token': token }
      });
  ...
  const aplicarAlternativa = async (alternativa) => {
    try {
      ...
      for (const h of alternativa.horarioGenerado) {
        try {
          await axios.post('http://localhost:5001/api/estudiante/matricular', { seccionId: h.seccion._id }, config);
        } catch (e) {
