...
function CursosManager() {
  ...
  useEffect(() => {
    fetchCursos();
    axios.get('/api/admin/carreras').then(res => setCarreras(res.data));
  }, []);

  const fetchCursos = async () => {
    setLoading(true);
    const res = await axios.get('/api/admin/cursos');
    setCursos(res.data);
    setLoading(false);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        await axios.put(`/api/admin/cursos/${editId}`, payload);
        setEditId(null);
      } else {
        await axios.post('/api/admin/cursos', payload);
      }
      setForm(EMPTY_FORM);
      fetchCursos();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`/api/admin/cursos/${id}`);
      fetchCursos();
    } catch (err) {
