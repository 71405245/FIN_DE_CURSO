...
function CarrerasManager() {
  ...
  const fetchCarreras = async () => {
    const res = await axios.get('http://localhost:5001/api/admin/carreras');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`http://localhost:5001/api/admin/carreras/${editId}`, form);
        setEditId(null);
      } else {
        await axios.post('http://localhost:5001/api/admin/carreras', form);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5001/api/admin/carreras/${id}`);
      fetchCarreras();
    } catch (err) {
